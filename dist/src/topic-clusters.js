 ...
 */
export function normalizeTopicKey(raw) {
    if (!raw)
 ...
 */
export function computeActiveWeight(f, w) {
    const x = w.a * (f.hit_rate || 0) +
        w.b * (f.recent_hits || 0) +
        w.c * (f.avg_answer_gain || 0) +
        w.d * (f.importance_mean || 0);
    return sigmoid(x);
}
const DEFAULT_WEIGHTS = { a: 0.4, b: 0.3, c: 0.2, d: 0.1 };
...
export class TopicClusters {
    ...
    async upsertTopic(input) {
        ...
        const prevVec = existing.vector || vector;
    ...
    async getActiveWeight(topicId, _scope) {
        ...
        if (!row)
            return 0;
        if (row.status === "archived" || row.status === "dormant")
            return 0;
