    ...
    register(api) {
        ...
        api.on("message_received", (event, ctx) => {
            try {
                ...
                if (!text.includes("[DREAMING]"))
                    return;
                api.logger.info(`memory-lancedb-pro: [DREAMING] system event received (sessionKey=${ctx?.sessionKey || event?.sessionKey || "unknown"}); triggering consolidation sweep`);
