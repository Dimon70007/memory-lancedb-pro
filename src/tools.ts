...
export function registerMemoryFeedbackTool(
  api: OpenClawPluginApi,
  context: ToolContext,
) {
  api.registerTool(
    (toolCtx) => {
      ...
      return {
        ...
        description:
          "Record explicit user feedback for a recalled memory candidate. Use 👍 (+1) when the memory was helpful/correct, 👎 (-1) when wrong/stale, 0 for neutral. Feeds the calibration loop (cluster_boost, importance, used_in_answer). T048-11.",
        parameters: Type.Object({
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, runtimeCtx) {
          const { candidateId, feedback } = params as {
            candidateId: string;
            feedback: number;
          };
          try {
